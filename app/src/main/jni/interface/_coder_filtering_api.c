...
static void emlrt_marshallIn(const emlrtStack *sp, const mxArray *y_raw, const
...
static void e_emlrt_marshallIn(const emlrtStack *sp, const mxArray *src, const
  emlrtMsgIdentifier *msgId, emxArray_real_T *ret)
{
  static const int32_T dims[1] = { 2954760 };
  ...
  char_T *identifier, emxArray_real_T *y)
{
  ...
  thisId.fParent = NULL;
  thisId.bParentIsCell = false;
  b_emlrt_marshallIn(sp, emlrtAlias(y_raw), &thisId, y);
 ...
 * Return Type  : void
 */
void filtering_api(const mxArray * const prhs[2], int32_T nlhs, const mxArray
                   *plhs[1])
{
  ...
  emxInit_real_T(&st, &y_raw, 1, true);
  emxInit_real_T(&st, &y_fil, 1, true);

  /* Marshall function inputs */
  y_raw->canFreeData = false;
  emlrt_marshallIn(&st, emlrtAlias(prhs[0]), "y_raw", y_raw);
  Fs = c_emlrt_marshallIn(&st, emlrtAliasP(prhs[1]), "Fs");

  /* Invoke the target function */
  filtering(y_raw, Fs, y_fil);

  /* Marshall function outputs */
  y_fil->canFreeData = false;
  plhs[0] = emlrt_marshallOut(y_fil);
  emxFree_real_T(&y_fil);
  emxFree_real_T(&y_raw);
